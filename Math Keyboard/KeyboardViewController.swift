    ...
    */
    ...
    override func viewDidLayoutSubviews() {
        ...
        if (lastLayoutBounds != nil && lastLayoutBounds == orientationSavvyBounds) {
            ...
            self.layout?.layoutKeys(self.currentMode, uppercase: uppercase, characterUppercase: characterUppercase, shiftState: .Disabled)
...
//            self.handleAutoPeriod(model)
    ...
    func setMode(mode: Int) {
        ...
        let uppercase = false
        let characterUppercase = (NSUserDefaults.standardUserDefaults().boolForKey(kSmallLowercase) ? uppercase : true)
        self.layout?.layoutKeys(mode, uppercase: uppercase, characterUppercase: characterUppercase, shiftState: .Disabled)
...
//    }
    ...
    func keyPressed(key: Key) {
        if let proxy = (self.textDocumentProxy as? UIKeyInput) {
            proxy.insertText(key.outputForCase(false))
        }
    }
